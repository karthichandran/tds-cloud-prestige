...
{
   public class GetForm16BReqListQuery : IRequest<IList<Form16BReqDto>>
    {
        public TdsPaymentFilter Filter { get; set; }
        public class GetForm16BReqListQueryHandler : IRequestHandler<GetForm16BReqListQuery, IList<Form16BReqDto>>
        {

            private readonly IApplicationDbContext _context;

            public GetForm16BReqListQueryHandler(IApplicationDbContext context)
            {
                _context = context;
            }

            public async Task<IList<Form16BReqDto>> Handle(GetForm16BReqListQuery request,
                CancellationToken cancellationToken)
            {
                ...
                var remittances = (from pay in _context.ClientPayment
                                    ...
                                    where (r!=null && r.ChallanDate!=null) && request.Filter.RemittanceStatusID.HasValue ||
                                          cpt.RemittanceStatusID == (int)ERemittanceStatus.TdsPaid
                                          && cp.OnlyTDS != true &&
